...
def normalize_item_name_universal(name):
    s = re.sub(r'^[○Ο●◎◆■□οo\-▪ㆍ･\s]+', '', str(name).strip())
    # Strip (성립전...), (간주...), (자체), (국비), (도비) etc.
    s = re.sub(r'[\(\[\-].*?(성립전|간주|경정).*?[\)\]]?', '', s).strip()
    s_clean = s.replace(" ", "")
...
def load_year_data_universal(year):
    filepath = os.path.join("data", f"budget_{year}.csv")
    ...
    # Step 2: Pure automated 1:1 replacement logic (NO YEAR CONDITIONAL, NO HARDCODED INDICES)
    ...
    for idx, r in enumerate(records):
        if r['정산 상태'] == '✅ 정산 포함':
            ...
            biz = str(r.get('세부사업명', '')).strip()
            tong = str(r.get('통계목명', '')).strip()
            r['norm_name'] = normalize_item_name_universal(r.get('산출근거명', ''))
...
df_2026 = load_year_data_universal(2026)
inc = df_2026[df_2026['정산 상태'] == '✅ 정산 포함']
...
print(f"============================================================")
print(f"=== 완전 자동 범용 엔진 (2025, 2026, 2027... 공용) 정산 결과 ===")
print(f"============================================================")
...
for d, c, t, diff in discrepancies[:10]:
