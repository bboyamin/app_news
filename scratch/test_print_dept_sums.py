...
def run_test():
    ...
    df_raw['amt_num'] = pd.to_numeric(amt_str, errors='coerce').fillna(0.0)
    ...
    for pdf_d, t_val in pdf_targets.items():
        ...
        if matched:
            total_167 += dept_sums[matched]

    print(f"Sum of 167 departments: {total_167:15,.0f} 천원")
    print(f"Sum of ALL included rows: {total_all_inc:15,.0f} 천원")
    print(f"Target PDF 167 sum:      {sum(pdf_targets.values()):15,.0f} 천원")
