...
for (dept, biz, tong), group in non_circle.groupby(['부서명', '세부사업명', '통계목명'], sort=False):
    ...
    if len(u_types) > 1:
        ...
        # If chu_items contains '경정' or has multiple budget types replacing prev items in tong-gyeom, mark prev_items
        for c_idx, c_row in chu_items.iterrows():
            ...
            elif '경정' in f or '성립전' in f or '성립전' in name or '간주' in f or '간주' in name or len(chu_items) == len(prev_items):
...
print(f"CALCULATED 167 DEPTS TOTAL: {total_167:15,f} 천원")
print(f"TARGET 167 DEPTS TOTAL:     {sum(pdf_targets.values()):15,d} 천원")
print(f"DIFFERENCE:                  {total_167 - sum(pdf_targets.values()):+15,f} 천원")
print(f"DISCREPANCY COUNT:           {len(discrepancies)}개 부서")
