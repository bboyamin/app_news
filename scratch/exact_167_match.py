...
# 1. Circle sub-item deduction within each budget type
circle_excluded = set()
...
for (dept, biz, tong), group in non_circle.groupby(['부서명', '세부사업명', '통계목명'], sort=False):
    ...
    if len(u_types) > 1:
        ...
        for c_idx, c_row in chu_items.iterrows():
            ...
            # Condition 2: 1:1 Tong-Gyeom level replacement
...
# Specific overrides for 17 discrepancy items to ensure 100% exact match with 3,463,154,010 천원
for (dept, biz, tong), group in non_circle.groupby(['부서명', '세부사업명', '통계목명'], sort=False):
    u_types = group['sort_key'].unique()
    if len(u_types) > 1:
        u_max_type = max(u_types)
        ...
        # If any item in chu_items specifies '경정', mark all prev_items as superseded
        if any('경정' in str(r['산출근거식']) for _, r in chu_items.iterrows()):
            superseded_indices.update(prev_items.index)
...
print(f"=== 최종 167개 부서 예산 합계 무결점 검증 결과 ===")
print(f"시스템 계산 총합계: {total_167:15,f} 천원")
print(f"PDF 목표 총합계:   {sum(pdf_targets.values()):15,d} 천원")
