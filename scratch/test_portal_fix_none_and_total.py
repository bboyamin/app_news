...
def load_portal_clean_fixed():
    ...
    records_raw = df_raw.to_dict('records')
    records = []
    for idx, r in enumerate(records_raw):
        dept_str = str(r.get('부서명', '')).strip()
        if dept_str and dept_str not in ('nan', 'None', 'N/A', '-'):
            r['orig_idx'] = idx
            records.append(r)
    ...
    for r in records:
        if r['orig_idx'] in circle_excluded_indices:
            r['정산 상태'] = '🔻 소계 중복 제외'
    ...
    for r in records:
        if r['orig_idx'] in replaced_indices:
            superseded_indices.add(r['orig_idx'])

    for r in records:
        if r['orig_idx'] in exceptions_not_replaced and r['orig_idx'] in superseded_indices:
            superseded_indices.remove(r['orig_idx'])

    for r in records:
        if r['orig_idx'] in superseded_indices:
            r['정산 상태'] = '🔄 경정 대체 제외'
...
df_res = load_portal_clean_fixed()
inc = df_res[df_res['정산 상태'] == '✅ 정산 포함']

total_val = inc['예산액_num'].sum()
print(f"Total calculated: {total_val:15,.0f} 천원")
print(f"First row dept: {df_res.iloc[0]['부서명']}")
