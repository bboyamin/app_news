...
for _, group in df_copy.groupby(['부서명', '세부사업명', '통계목명', '예산구분'], sort=False):
    ...
    i = 0
    while i < n:
        curr_lvl = get_symbol_level(records[i]['산출근거명'])
        if curr_lvl == 1:
            j = i + 1
            while j < n:
                next_lvl = get_symbol_level(records[j]['산출근거명'])
                if next_lvl == 1: break
...
with open('scratch/check_res.txt', 'w', encoding='utf-8') as f:
    f.write(f'TOTAL_SUM_ALL={total_sum_all:,.0f}\n')
    f.write(f'TOTAL_SUM_167={total_sum_167:,.0f}\n')
    f.write(f'PDF_TARGET_SUM={sum(pdf_targets.values()):,.0f}\n')
    f.write(f'DISCREPANCY_COUNT={len(discrepancies)}\n')
    for d, c, t, diff in sorted(discrepancies, key=lambda x: abs(x[3]), reverse=True):
        f.write(f'DIFF: {d:25s} | system={c:12,d} | pdf={t:12,d} | diff={diff:+12,d}\n')

print('Done writing scratch/check_res.txt')
