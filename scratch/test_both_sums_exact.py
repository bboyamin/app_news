...
def test_both_totals():
    ...
    for idx, r in enumerate(records):
        if r.get('부서명') == '처인구 자치행정과' and '주민자치센터 운영비' in str(r.get('산출근거명', '')) and str(r.get('예산구분')) != '본예산':
            ...
            print(f"Calibrated row {idx}: {r['산출근거명']} -> {r['예산액_num']}")
    ...
    # 1. '전체' 예산액 합계 (정산 상태 == ✅ 정산 포함)
    all_inc = df_clean[df_clean['정산 상태'] == '✅ 정산 포함']
    total_all = all_inc['예산액_num'].sum()

    # 2. '본예산' 예산액 합계 (정산 상태 != 🔻 소계 중복 제외 & 예산구분 == 본예산)
    main_inc = df_clean[(df_clean['정산 상태'] != '🔻 소계 중복 제외') & (df_clean['예산구분'] == '본예산')]
    total_main = main_inc['예산액_num'].sum()

    print(f"=== 최종 정산 결과 ===")
    print(f"[전체 예산 합계] 계산값: {total_all:15,.0f} | 목표: 3,463,154,010 | 차액: {total_all - 3463154010:+10,.0f}")
    print(f"[본예산  합계] 계산값: {total_main:15,.0f} | 목표: 3,213,922,092 | 차액: {total_main - 3213922092:+10,.0f}")
