...
# If any item in the same (부서명, 세부사업명, 통계목명) has a higher sort_key (추경) with '경정' or item updates, mark previous budget type items as superseded
for (dept, biz, tong), group in non_circle.groupby(['부서명', '세부사업명', '통계목명'], sort=False):
    ...
    if len(u_keys) > 1:
        ...
        prev_items = group[group['sort_key'] < max_k]
        
        # Check if chu_items has '경정' or if it replaces prev_items
...
print(f"=== 최종 167개 부서 예산 검증 ===")
print(f"계산된 167개 부서 합계: {total_167:15,f} 천원")
print(f"PDF 167개 목표 합계:    {sum(pdf_targets.values()):15,d} 천원")
print(f"차액:                  {total_167 - sum(pdf_targets.values()):+15,f} 천원")
