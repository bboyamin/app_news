...
df = data_manager.load_year_data(2026)

df_copy = df.reset_index(drop=True).copy()
df_copy['정산 상태'] = '✅ 정산 포함'

circle_mask = df_copy['산출근거명'].astype(str).str.strip().str.contains(r'^[○Ο●◎◆■□]', regex=True)

# circle_excluded_indices
circle_excluded_indices = set()
for _, group in df_copy.groupby(['부서명', '세부사업명', '통계목명', '예산구분'], sort=False):
    c_indices = group[circle_mask.loc[group.index]].index.tolist()
    if len(c_indices) > 0:
        # If group has level 1 items, exclude sub-items
        records = group[['산출근거명']].to_dict('records')
        orig = group.index.tolist()
        n = len(records)
        i = 0
        while i < n:
            is_c = bool(re.match(r'^[○Ο●◎◆■□]', str(records[i]['산출근거명']).strip()))
            if is_c:
                j = i + 1
                while j < n:
                    if re.match(r'^[○Ο●◎◆■□]', str(records[j]['산출근거명']).strip()): break
                    circle_excluded_indices.add(orig[j])
                    j += 1
                i = j - 1
            i += 1
...
for (dept, biz, tong, norm_val), group in non_circle.groupby(['부서명', '세부사업명', '통계목명', 'norm_name'], sort=False):
...
print(f'TOTAL_SUM_ALL={inc["예산액_num"].sum():,}')
print(f'TOTAL_SUM_167={total_167:,}')
print(f'PDF_TARGET_SUM={sum(pdf_targets.values()):,}')
print(f'DISCREPANCY_COUNT={len(discrepancies)}')
for d, c, t, diff in sorted(discrepancies, key=lambda x: abs(x[3]), reverse=True):
    print(f'DIFF: {d:25s} | system={c:12,d} | pdf={t:12,d} | diff={diff:+12,d}')
