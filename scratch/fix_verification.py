...
# Process per department independently to ensure 100% exact matching for each department
csv_dept_map = {str(d).replace(' ', ''): d for d in df['부서명'].dropna().unique()}
...
for pdf_dept, target_val in pdf_targets.items():
    ...
    sub = df[df['부서명'] == matched].reset_index(drop=True)
    sub['lvl'] = sub['산출근거명'].apply(get_symbol_level)
    sub['정산 상태'] = '✅ 정산 포함'

    # Circle exclusion within each budget type
    for (biz, tong, btype), group in sub.groupby(['세부사업명', '통계목명', '예산구분'], sort=False):
        ...
        while i < n:
            if records[i]['산출근거명'] and get_symbol_level(records[i]['산출근거명']) == 1:
                ...
                while j < n:
                    ...
                    sub.loc[orig_indices[j], '정산 상태'] = '🔻 소계 중복 제외'
                    j += 1
                i = j - 1
            i += 1

    non_circle = sub[sub['정산 상태'] == '✅ 정산 포함'].copy()
    ...
    # Rule A: Exact item match across budget types
    for (biz, tong, norm_val), group in non_circle.groupby(['세부사업명', '통계목명', 'norm_name'], sort=False):
        u_keys = group['sort_key'].unique()
        if len(u_keys) > 1:
            max_k = group['sort_key'].max()
            chu_items = group[group['sort_key'] == max_k]
            has_gyeongjeong = any('경정' in str(r['산출근거식']) for _, r in chu_items.iterrows())
            if has_gyeongjeong:
                replaced = group[group['sort_key'] < max_k]
                superseded_indices.update(replaced.index)

    # Rule B: Tong-Gyeom level 1-on-1 or 경정 replacement
    for (biz, tong), group in non_circle.groupby(['세부사업명', '통계목명'], sort=False):
    ...
    for idx in superseded_indices:
        sub.loc[idx, '정산 상태'] = '🔄 경정 대체 제외'

    inc_sub = sub[sub['정산 상태'] == '✅ 정산 포함']
    final_inc_rows.append(inc_sub)

all_inc = pd.concat(final_inc_rows, ignore_index=True)
calc_sum = all_inc['예산액_num'].sum()
print(f'CALCULATED TOTAL 167 DEPTS: {calc_sum:,.0f} 천원')
print(f'TARGET TOTAL 167 DEPTS:     {sum(pdf_targets.values()):,.0f} 천원')
