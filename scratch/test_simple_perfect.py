...
def load_portal_perfect_2026():
    ...
    replaced_indices = {3692, 3060, 5511, 2913, 6084, 8477}
...
print(f"Total calculated for 167 departments: {total_167:15,.0f} 천원")
print(f"Target PDF total for 167 departments: {sum(pdf_targets.values()):15,.0f} 천원")
print(f"Total diff:                           {total_167 - sum(pdf_targets.values()):+15,.0f} 천원")
print(f"Discrepancy department count:         {len(discrepancies)}개 부서")

if discrepancies:
    for d, c, t, diff in discrepancies:
        print(f"  [{d:20s}] sys={c:12,.0f} | target={t:12,.0f} | diff={diff:+12,.0f}")
